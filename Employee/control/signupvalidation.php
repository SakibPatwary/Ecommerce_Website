...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
    ...
    if(empty($fname) || empty($lname) || empty($nid) || empty($nnid) || empty($gender) || empty($email) || empty($dob) || empty($password) || strlen($password)<6 || strlen($phone)<11 ||
      strlen($nid)<11 || strlen($nnid)<11 || empty($c_password) || $password !== $c_password)
    {
        $error = "Error!!!";
    }
